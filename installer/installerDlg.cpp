...
BEGIN_MESSAGE_MAP(CinstallerDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDOK, &CinstallerDlg::OnBnClickedOk)
END_MESSAGE_MAP()
...
void CinstallerDlg::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	char name[20],pwd[20];
	this->name.GetWindowTextA(name,19);
	this->pwd.GetWindowTextA(pwd,19);
	string n(name);
	string p(pwd);
	fstream file;
	...
	file<<"\n";
	file.close();
	file.open("Officer.dat",ios::out);
	file<<"\n";
	file.close();
	file.open("BookInformation.txt",ios::out);
	file<<"\n";
	file.close();
	file.open("connect.dat",ios::out);
	file<<"\n";
	file.close();
	CDialogEx::OnOK();
}
